...
[Boente G, Salibian-Barrera M, Vena P. Robust estimation for semi-functional linear regression models. To appear in *Computational Statistics and Data Science*](https://doi.org/10.1016/j.csda.2020.107041). 
...
- `w`: the varying coefficients for the non-parametric term (w = 1 for non varying coefficients);
- `range_func_term`: a vector of B-spline basis sizes
to try for the functional regression coefficient;
- `range_nonparam_term`: a vector of B-spline basis sizes
to try for the non-parametric component;
- `spl_order`: the order of the B-splines;
- `loss_fun`: the loss function to be minimized; and
- `model_selection`: Function for the criterion for model selection.
...
a <- FPLMBsplines(y = y, x = x, u = u, t = t, range_freq = range_beta, range_spl = range_eta, spl_order = spl_order, loss_fun=loss_lmrob)
```
To fit the M and LS estimators we simply change the argument
`fLoss`:
```{r nonrobust, tidy=TRUE, warning=FALSE, message=FALSE, cache=TRUE}
d <- FPLMBsplines(y = y, x = x, u = u, t = t, range_freq = range_beta, range_spl = range_eta, spl_order = spl_order, loss_fun=loss_huang)

b <- FPLMBsplines(y = y, x = x, u = u, t = t, range_freq = range_beta, range_spl = range_eta, spl_order = spl_order, loss_fun=loss_ls, trace=FALSE)
```
...
u_order <- order(u)
ma <- max(rra <- c(a$est_npt_fun, b$est_npt_fun, d$est_npt_fun))
mi <- min(rra)
plot(u[u_order], a$est_npt_fun[u_order], type = "l", xlab = "Protein", pch = 1, lwd = 6, col = "black", ylab = expression(hat(eta)), lty=1, ylim=c(mi, ma))
lines(u[u_order], d$est_npt_fun[u_order], lwd = 6, col = "tomato3")
lines(u[u_order], b$est_npt_fun[u_order], lwd = 6, col = "skyblue3")
...
ma <- max(rrb <- c(a$est_ft_fun, b$est_ft_fun, d$est_ft_fun))
mi <- min(rrb)
plot(t, a$est_ft_fun, col = "black", lwd = 6, type = "l", xlab = "Wavelength", ylab = expression(hat(beta)), lty=1, ylim=c(mi, ma))
lines(t, d$est_ft_fun, col = "tomato3", lwd = 6, lty=1)
lines(t, b$est_ft_fun, col = "skyblue3", lwd = 6, lty=1)
...
u2  <- u[ -outl ]
x2  <- x[ -outl,]
d2 <- FPLMBsplines(y = y2, x = x2, u = u2, t = t, range_freq = range_beta, range_spl = range_eta, spl_order = spl_order, loss_fun=loss_huang)

b2 <- FPLMBsplines(y = y2, x = x2, u = u2, t = t, range_freq = range_beta, range_spl = range_eta, spl_order = spl_order, loss_fun=loss_ls)
```
...
mi <- min(rra)
plot(u[u_order], a$est_npt_fun[u_order], type = "l", xlab = "Protein", pch = 1, lwd = 6, col = "black", ylab = expression(hat(eta)), lty=1, ylim=c(mi, ma))
lines(u2[u2_order], d2$est_npt_fun[u2_order], lwd = 6, col = "tomato3")
lines(u2[u2_order], b2$est_npt_fun[u2_order], lwd = 3, col = "skyblue3")
...
mi <- min(rrb)
plot(t, a$est_ft_fun, col = "black", lwd = 6, type = "l", xlab = "Wavelength", ylab = expression(hat(beta)), lty=1, ylim=c(mi, ma))
lines(t, d2$est_ft_fun, col = "tomato3", lwd = 6, lty=1)
lines(t, b2$est_ft_fun, col = "skyblue3", lwd = 3, lty=1)
